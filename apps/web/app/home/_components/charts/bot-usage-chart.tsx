...
type Bot = 'bot_1' | 'bot_2' | 'bot_3' | 'bot_4';
...
const botsData = [
  { name: 'Bot 1', value: 'bot_1' },
  { name: 'Bot 2', value: 'bot_2' },
  { name: 'Bot 3', value: 'bot_3' },
  { name: 'Bot 4', value: 'bot_4' },
];
...
export function BotUsageChart() {
  const [selectedWeek, setSelectedWeek] = useState<Week>('last_week');
  const [selectedBot, setSelectedBot] = useState<Bot>('bot_1');
  ...
  const chartData = useMemo(
    () => [
      {
        name: 'usedMinutes',
        value: currentWeekData.bots[selectedBot].usedPercentage,
        fill: 'var(--color-brand-800)',
      },
      {
        name: 'remainingMinutes',
        value: currentWeekData.bots[selectedBot].remainingPercentage,
        fill: 'var(--color-brand-600)',
      },
    ],
    [currentWeekData.bots, selectedBot],
  );
  ...
  return (
    <Card className="bg-brand-slate-50 border-brand-slate-100 rounded-md p-4 pb-0">
      ...
      <CardContent className="w-full p-0 pt-1.5">
        <Tabs
          value={selectedBot}
          onValueChange={(v) => setSelectedBot(v as Bot)}
          className="w-full"
        >
